...
def createSourceConnectionWithDelimiter(dataflow_name, base_connection_id, connection_spec_id, source_file_path, sandbox_name, security_credentials):
    SOURCE_CONN_URL = "https://platform.adobe.io/data/foundation/flowservice/sourceConnections"
    req_headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
...
def configureDataFlow(fileset_name, sandbox_name, security_credentials, config_json):
    try:
        ...
        hasColumnDelimiter = tautils.hasProperty(
            "abbVie.dataload." + sandbox_name + ".datasetDetails." + fileset_name + ".columnDelimiter", config_json)
        ...
        for input_file in source_file_path_array:
            if hasColumnDelimiter:
                DATAFLOW_DETAILS_DICT["SOURCE_CONN_ID"] = createSourceConnectionWithDelimiter(fileset_name,
...
if __name__ == "__main__":
    ...
    configureDataFlow("AgentInteractions", TARGET_ENVIRONMENT, security_credentials, config_json)
